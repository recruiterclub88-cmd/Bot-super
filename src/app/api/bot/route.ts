...
export async function POST(req: Request) {
  ...
  // --- DB SAFETY CHECK (Lazy Init) ---
  const supabaseAdmin = getSupabaseAdmin();
  if (!supabaseAdmin) {
    console.error('❌ [Webhook] Supabase client missing. Cannot process message.');
  ...
  if (contactErr) {
    console.error('[Webhook] Contact fetch error:', contactErr);
  ...
  if (inErr) {
    ...
    console.error('[Webhook] Message insert error:', inErr);
  ...
  try {
    ...
    console.error('[Webhook] GreenAPI send error:', e);
  ...
  if (outErr) {
    console.error('[Webhook] Outbound message save error:', outErr);
  }
